	...
	public function new(curStage)
	{
		...
		if (PlayState.determinedChartType == "FNF")
		{
			...
			switch (CoolUtil.spaceToDash(PlayState.SONG.song.toLowerCase()))
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'breakback' | 'dumpster-fire' | 'blake' | 'b64' | 'brobots':
					curStage = 'blakechungus';
				default:
					curStage = 'stage';
			}
		...
		switch (curStage)
		{
			...
			case 'blakechungus':
				...
				var repositionShit = -200;
				//constant
				var groundbg:FNFSprite = new FNFSprite(repositionShit,0).loadGraphic(Paths.image('blakesheets/constant/Land'));
				var trashbg:FNFSprite = new FNFSprite(repositionShit,0).loadGraphic(Paths.image('blakesheets/constant/Trash'));
				//cameos
				bgFriends = new BlakeFriends(repositionShit,0);
				var should_add_friends = false;
				//var bgFriends = new FNFSprite(repositionShit,0);
				//bgFriends.frames = Paths.getSparrowAtlas('characters/Cameos');
				//set time
				var blake_BG_type = '1'; // 1 = morning, 2 = dusk, 3 = night
				switch (PlayState.SONG.song.toLowerCase()){
					case 'b64': //sunset
						blake_BG_type = '2';
					case 'wagabobo2': //night
						blake_BG_type = '3';
					case 'brobots': //brobots
						blake_BG_type = '3';
						should_add_friends = true;
					default: //day
						blake_BG_type = '1';
				}
				//varies 
				var blake_BG_sky = new FNFSprite(repositionShit,0).loadGraphic(Paths.image('blakesheets/Stage' + blake_BG_type + '/Sky'));
				var blake_BG_sea = new FNFSprite(repositionShit,0).loadGraphic(Paths.image('blakesheets/Stage' + blake_BG_type + '/Sea'));
				var blake_BG_crane = new FNFSprite(repositionShit,0).loadGraphic(Paths.image('blakesheets/Stage' + blake_BG_type + '/Crane'));
				var blake_BG_water = new FNFSprite(repositionShit,0).loadGraphic(Paths.image('blakesheets/Stage' + blake_BG_type + '/Water'));
				//scrollfactor
				blake_BG_sky.scrollFactor.set(0.55, 0.55);
				blake_BG_sea.scrollFactor.set(0.75, 0.60);
				blake_BG_crane.scrollFactor.set(0.75, 0.60);
				blake_BG_water.scrollFactor.set(0.75, 0.60);
				groundbg.scrollFactor.set(0.95, 0.85);
				trashbg.scrollFactor.set(0.95, 0.85);
				bgFriends.scrollFactor.set(0.95, 0.85);

				//order should be: sky>sea>water>crane>land>trash
				add(blake_BG_sky);
				add(blake_BG_sea);
				add(blake_BG_water);
				add(blake_BG_crane);
				add(groundbg);
				add(trashbg);
				if (should_add_friends) {
					add(bgFriends);
				}
				

				//scale up ingame
				var widShit = Std.int(blake_BG_sky.width * 6);
				blake_BG_sky.setGraphicSize(widShit);
				blake_BG_sea.setGraphicSize(widShit);
				blake_BG_crane.setGraphicSize(widShit);
				blake_BG_water.setGraphicSize(widShit);
				groundbg.setGraphicSize(widShit);
				trashbg.setGraphicSize(widShit);
				bgFriends.setGraphicSize(widShit);

				blake_BG_sky.updateHitbox();
				blake_BG_sea.updateHitbox();
				blake_BG_crane.updateHitbox();
				blake_BG_water.updateHitbox();
				groundbg.updateHitbox();
				trashbg.updateHitbox();
				bgFriends.updateHitbox();
	...
	public function repositionPlayers(curStage, boyfriend:Character, dad:Character, gf:Character):Void
	{
		...
		switch (curStage)
		{
			...
			case 'blakechungus':
				gf.x += 180;
				gf.y += 300;
				if (PlayState.SONG.song.toLowerCase() == "brobots") {
					boyfriend.x += 180;
					boyfriend.y += 200;
					gf.visible = false;
				} else {
