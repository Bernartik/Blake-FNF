...
import gameObjects.userInterface.FcIcon;
...
class FreeplayState extends MusicBeatState
{
	...
	private var fcArray:Array<FcIcon> = [];
	...
	override function create()
	{
		...
		**/
		...
		for (i in folderSongs)
		{
			...
			{
				if (!((i.toLowerCase() == "test") || (i.toLowerCase() == "tutorial") || (i.toLowerCase() == "brobots" && Main.goodEnd == false))) {
					var icon:String = 'gf';
		...
		for (i in 0...songs.length)
		{
			...
			//getting if the song was fc'd or nah
			var bigshitter:Bool = Highscore.getFC(songs[i].songName.toLowerCase(),
				CoolUtil.difficultyArray.indexOf(existingDifficulties[i][1])); 

			var icon:HealthIcon = new HealthIcon(songs[i].songCharacter);
			var icon2:FcIcon = new FcIcon(bigshitter);
			icon.sprTracker = songText;
			icon2.sprTracker = songText;
		...
		//waga baga bobo
		//debugText0 = new FlxText(30, 5, 0, "", 30);
		//debugText0.setFormat(Paths.font("vcr.ttf"), 30, FlxColor.WHITE);
		//debugText1 = new FlxText(30, 35, 0, "", 30);
		//debugText1.setFormat(Paths.font("vcr.ttf"), 30, FlxColor.WHITE);
		//debugText2 = new FlxText(30, 65, 0, "", 30);
		//debugText2.setFormat(Paths.font("vcr.ttf"), 30, FlxColor.WHITE);
	...
	override function update(elapsed:Float)
	{
		...
		//debugText0.text = "Song: " + songs[curSelected].songName;
		//debugText1.text = "Intended FC: " + intendedFC;
		//debugText2.text = "Current Selection: " + curSelected;

		//fcArray[curSelected].songWasFc = intendedFC;
		//fc update stuff
		for (i in 0...fcArray.length)
		{
			fcArray[i].curframeidk += 0.1;
			if (fcArray[i].curframeidk >= 9) {
				fcArray[i].curframeidk = 0;
			}
			fcArray[i].animation.curAnim.curFrame = Std.int(fcArray[i].curframeidk);//manually changing frame cause for some reason animation play didn't work with the icon
		}
	...
	function changeDiff(change:Int = 0)
	{
		...
		fcArray[curSelected].songWasFc = intendedFC;
		if (fcArray[curSelected].songWasFc) {
			fcArray[curSelected].alpha = 1;
		} else {
			fcArray[curSelected].alpha = 0;
		}
