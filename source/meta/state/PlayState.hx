...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if ((curStage.startsWith("school") || curStage.startsWith("blake")) && determinedChartType == "FNF")
			assetModifier = 'pixel';
				...
				else
				{
					...
					switch (curStage)
					{
						...
						case 'schoolEvil':
							...
							getCenterY = char.getMidpoint().y - 200;
						case 'blakechungus':
							if (boyfriend.curCharacter == "blake") {
	...
	function endSong():Void
	{
		...
		vocals.volume = 0;
		if (SONG.validScore)
			Highscore.saveScore(SONG.song, songScore, storyDifficulty, misses);
		...
		else
		{
			...
			if ((storyPlaylist.length <= 0) && (!endSongEvent))
			{
				...
				// change to the menu state  SIKE not anymore
				//	Main.switchState(this, new StoryMenuState());

				// waga baga bobo
				//	go to week end cutscene if blake (mostly for debug, tutorial will be removed in final version)
				if (dadOpponent.curCharacter == "blake") {
					var EndWeekScene = new EndWeekState();
					if (misses > 20) { //if you miss too much you get the bad end
						EndWeekScene.sceneNum = "2";
					} else { //else you unlock 4th song
						Main.goodEnd = true;
						FlxG.save.data.goodEnd = true;
					}
					Main.switchState(this, EndWeekScene);
				} else {
					Main.switchState(this, new StoryMenuState());
				}
